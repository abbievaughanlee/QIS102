# second_moment.py
# simulate variance distribution for dice (only integer values)
...
def main():
    ...
    for idx, sides in enumerate(dice):
        for n in range(len(magic[0])):  # 0 <= n < 21 (# items in 1st row of matrix)
            ...
            #FORMULA FOR CONTINUOUS DISTRIBUTION (WRONG): magic[idx, n] = (sides - 1) ** 2 / var  # calculate magic number 
            #instead, use the formula for discrete distribution to limit to only integers. This ensures that the magic number is 12
            magic[idx, n] = (sides**2 - 1) / var
