...
plt.bar(r_values, dist2, label="p = 0.5") # 50 percent success rate
plt.bar(r_values, dist3, label="p = 0.9") # 90 percent success rate
plt.title(f"Binomial Distribution (n={n} trials)")
plt.xlabel("Number of Successes")
plt.ylabel("Probability")
#certain things you can call on pyplot that are only invocable on the ax or xaxis objects
ax = plt.gca() #pointer to axis (get current axis)
ax.legend(loc="upper left") 
ax.xaxis.set_major_locator(MultipleLocator(5)) # locators for major and minor ticks in pyplot
plt.show()
